...
人在上海，已经被关了五十多天了。物理上收到限制后，就被迫在数码空间中进行更多的工作、娱乐和信息交流。这里分享一些我减少噪音、提升效率和获得内心平静的经验心得。
...
安装这个Mobile Wikipedia插件后，可以把访问的维基百科页面重定向到相应移动端版本（其实就是在链接中增加一个m），移动版维基百科的版式设计更好，阅读体验更佳。

### 转换至简体

有的时候我会访问一些繁体中文的博客，比如电脑玩物（数字工具和效率）以及王孟源（国际政治），阅读时还是用插件转换成简体字后更方便。这个插件的好处是一点权限都没有要，非常的干净，让人放心。
